...
class SpamManagmentTest extends TestCase
{
    ...
    public function test_marking_an_idea_as_spam_does_not_works_when_user_has_not_authorization()
    {
        $idea = Idea::factory()->create();
    ...
    public function test_marking_an_idea_as_spam_does_not_shows_on_menu_when_user_has_not_authorization()
    {
        $idea = Idea::factory()->create();
    ...
    public function test_marking_an_idea_as_not_spam_does_not_shows_on_menu_when_user_has_not_authorization()
    {
        $idea = Idea::factory()->create();
    ...
    public function test_marking_an_idea_as_not_spam_works_when_user_has_authorization()
    {
        ...
        Livewire::actingAs($user)
            ...
            ->call('resetSpamCounter')
            ->assertEmitted('ideaWasMarkedAsNotSpam');

        $this->assertEquals(0, Idea::first()->spam_reports);
    }

    public function test_spam_reports_count_shows_on_ideas_index_page_if_logged_in_as_admin()
    {
        $user = User::factory()->create([
            'email' => 'dato@redberry.ge'
        ]);
